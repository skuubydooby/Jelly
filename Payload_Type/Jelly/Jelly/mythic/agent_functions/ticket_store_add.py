...
class ticket_store_addCommand(CommandBase):
    ...
    description = "Add a kerberos ticket to the agents internal ticket store. Tickets are topped into sacrificial processes when you're impersonating a token (make_token / steal_token). This is because you have a new logon session to put the tickets into without overriding your existing tickets. For safety, do a make_token with junk creds first."
    version = 2
    author = "@drago-qcc"
